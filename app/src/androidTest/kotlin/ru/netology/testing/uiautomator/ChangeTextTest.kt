...
//    }
    ...
    @Test
    fun testEnteringEmptyString() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        val sameText = device.findObject(By.res(packageName, "textToBeChanged")).text
        device.findObject(By.res(packageName, "userInput")).text = emptyLine
        device.findObject(By.res(packageName, "buttonChange")).click()

        val result = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(result, sameText)
    }

    @Test
    fun testOpeningNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
